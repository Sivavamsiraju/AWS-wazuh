...
echo "Wazuh-Worker: Starting process." > /tmp/deploy.log
...
cluster_configuration(){
    ...
    sshpass -f $ssh_password scp -o "StrictHostKeyChecking=no" $ssh_username@192.168.1.205:/home/$ssh_username/cluster_key.txt /home/$ssh_username/

    cluster_key = `cat cluster_key.txt`
    sed -i -e "s#<node_name>node01</node_name>#<node_name>worker-node</node_name>#" /var/ossec/etc/ossec.conf
    sed -i -e "s#<bind_addr>0.0.0.0</bind_addr>#<bind_addr>192.168.1.206</bind_addr>#" /var/ossec/etc/ossec.conf
    ...
    sed -i -e "s#<node_type>master</node_type>#<node_type>worker</node_type>#" /var/ossec/etc/ossec.conf
